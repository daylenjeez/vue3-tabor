...
为了确保 i18n 功能正常工作，我们为国际化功能添加了专门的单元测试。运行以下命令可执行测试：
...
1. **核心 i18n 工具测试** (`src/tests/i18n.test.ts`)：
   - 语言切换功能
   - 翻译检索功能
   - 默认语言处理

2. **下拉菜单组件翻译测试** (`src/tests/components/dropdown.test.tsx`)：
   - 默认中文显示
   - 英文设置下的文本显示
   - 语言切换时文本更新

3. **语言切换组件测试** (`src/tests/components/languageSwitch.test.tsx`)：
   - 语言切换按钮渲染
   - 点击切换语言的功能
   - 不同语言下的按钮显示

4. **整合测试** (`src/tests/i18n-integration.test.tsx`)：
   - Tabor 组件中语言属性的传递
   - 语言切换按钮的显示/隐藏
   - 默认语言处理
...
1. 在 `src/utils/i18n.ts` 中的翻译对象中添加新的键值对
2. 更新 `TranslationKey` 类型
3. 在组件中使用 `t('新键名')` 获取翻译
4. 为新添加的翻译编写测试用例
